...
function fourier_test()
    pass = true
    ω_arr = collect(linspace(0.0,100.0,200))
    t_arr = ω_to_t(ω_arr)
    N = length(ω_arr) - 1
    t0 = t_arr[N]

    # test invertablity
    ω_arr ≈ t_to_ω(ω_to_t(ω_arr))
    fs = rand(-1:0.01:1.,2N+1,1)
    pass = pass && fs ≈ frequency_to_time(time_to_frequency(fs,ω_arr),ω_arr)

    # Gaussian to Gaussian
    as = [0.5];
    Fs = Complex{Float64}[exp(-a*ω^2)*exp(im*t0*ω) for ω in ω_arr, a in as]
    fs =  frequency_to_time(Fs,ω_arr,t_arr);
    true_fs = (1/(2pi))*[sqrt(pi/a)*exp(-(t-t0)^2/(4*a)) for t in t_arr, a in as]
    pass = pass && fs ≈ true_fs
    pass = pass && Fs ≈ time_to_frequency(fs,ω_arr,t_arr)

    # sinc to rectangle
    rect(t) = (abs(t)<0.5) ? 1.0:0.
    as = [0.7]; #only positive values
    Fs = Complex{Float64}[ (ω == 0.) ? 1:sin(a*pi*ω)*exp(im*t0*ω)/(a*pi*ω) for ω in ω_arr, a in as]
    ...
    Fs_trap = time_to_frequency(fs,ω_arr; method=:trapezoidal)
    Fs =time_to_frequency(fs,ω_arr)
    ...
    # ω_arr = collect(linspace(0.0,10.0,200)) # need fine mesh near ω=0
    # dω = (ω_arr[2]-ω_arr[1])
    # N = length(ω_arr)-1;

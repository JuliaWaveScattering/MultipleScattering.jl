...
@testset "Time Result" begin
    ...
    # invertability of dft
        ...
        timres = run(sim, x_vec, ω_vec; result_in_time=true, impulse=TimeDeltaFunctionImpulse(0.0), method=:dft)
    ...
    # Compare dft and trapezoidal integration
        ...
        simres = run(sim, x_vec, ω_vec)
        timres1 = frequency_to_time(simres; method=:trapezoidal, impulse=TimeDeltaFunctionImpulse(0.0))
        timres2 = frequency_to_time(simres; method=:dft, impulse=TimeDeltaFunctionImpulse(0.0))
        ...
        t_vec = 0.:0.5:20.
        simres = run(sim, x_vec, ω_vec)
        timres1 = frequency_to_time(simres; t_vec = t_vec,
            method=:trapezoidal, impulse=GaussianImpulse(maximum(simres.ω)))
        timres2 = frequency_to_time(simres; t_vec = t_vec,
            method=:dft, impulse=GaussianImpulse(maximum(simres.ω)))
        norm(field(timres1) - field(timres2))/norm(field(timres1))
